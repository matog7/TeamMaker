...
const formations: Formation[] = [
  ...
  },
  {
    ...
    positions: [
      ...
      { x: 50, y: 90 },
      // Défenseurs
      { x: 20, y: 65 }, { x: 40, y: 70 }, { x: 60, y: 70 }, { x: 80, y: 65 },
      // Milieux
      { x: 35, y: 50 }, { x: 65, y: 50 }, { x: 50, y: 35 },
      // Attaquants
      { x: 20, y: 25 }, { x: 80, y: 25 }, { x: 50, y: 15 }
    ]
  },
  {
    id: '41212',
    ...
    positions: [
      ...
      { x: 50, y: 90 },
      // Défenseurs 
      { x: 15, y: 70 }, { x: 40, y: 75 }, { x: 60, y: 75 }, { x: 85, y: 70 },
      // Milieux
      { x: 15, y: 45 }, { x: 50, y: 60 }, { x: 50, y: 35 }, { x: 85, y: 45 },
      // Attaquants
      { x: 35, y: 15 }, { x: 65, y: 15 }
    ]
  },

  // Autres formations à ajouter...
]
...
function App() {
  ...
  const [form, setForm] = useState<{ name: string; rating: string; potential: string; photo: string }>({ name: '', rating: '', potential: '', photo: '' })
  ...
  return (
    <div className="">
      ...
      <div id="interface" className="w-screen flex flex-col lg:flex-row items-center justify-center">
        <div id='left-column' className='flex flex-col items-center justify-center gap-4 w-full'>
              ...
              <div className="space-y-2">
                {formations.map((formation) => (
                  <button
                    ...
                    className={`w-full p-2 rounded transition-colors ${selectedFormation.id === formation.id
          ...
          <div className="bg-white rounded-lg shadow-lg p-4 mb-4 flex flex-col w-full max-w-xs">
            ...
            <h2 className="text-xl font-semibold text-gray-900 mb-4">Joueurs sur le terrain</h2>
            <ul className="space-y-3">
              {selectedFormation.positions.map((_, idx) => {
                const player = players[idx];
                if (!player) return null;
                return (
                  <li key={idx} className="flex items-center gap-3 bg-gray-100 rounded p-2">
                    {player.photo && (
                      <img src={player.photo} alt={player.name} className="w-10 h-10 rounded-full object-cover border border-gray-400" />
                    )}
                    <div className="flex-1">
                      <div className="font-bold text-gray-900 text-sm">{player.name}</div>
                      <div className="text-sm text-gray-700 font-semibold">
                        Note : <p className={`inline-block ${player.rating > 70 && player.rating <= 80 ? 'text-green-600' : 'text-yellow-500'}`}>{player.rating}</p>,
                        Potentiel: <p className={`inline-block ${player.potential > 70 && player.potential <= 80 ? 'text-green-600' : player.potential > 80 ? 'text-green-800' : 'text-yellow-500'}`}>{player.potential}</p>
                      </div>
                    </div>
                  </li>
                );
          ...
          {Object.keys(players).length > 0 && (
            <div className="rounded-lg p-4 mb-4 flex flex-col">
              <button className="bg-white/90 text-white-900 rounded-lg p-2">Sauvegarder équipe</button>
            </div>
          )}
        </div>
        <div id='right-column' className='flex flex-col items-center justify-center gap-4 w-full'>
          {/* Terrain de football */}
                ...
                >
                  {players[index] ? (
                    <div className="flex flex-col items-center justify-center">
                      <div className="font-bold text-sm text-gray-900">{players[index].name}</div>
                      <div className={`text-xs ${players[index].rating > 70 ? 'bg-green-600' : 'bg-yellow-500'} text-white rounded-full px-2 py-0.5 mt-1`}>
                        {players[index].rating}
                      </div>
                      {players[index].photo && (
                        <img src={players[index].photo} alt="photo" className="w-8 h-8 rounded-full mx-auto mt-1 object-cover border border-gray-400" />
                      )}
                    </div>
                  ) : (
              ...
              {modalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                  <div className="bg-white rounded-lg p-6 w-80 relative border-2 border-gray-700">
                    ...
                    <h3 className="text-lg font-bold mb-4 text-gray-900">{players[editingIndex!] ? 'Modifier' : 'Ajouter'} un joueur</h3>
                    <form onSubmit={handleSubmit} className="space-y-3">
                      <div>
                        <label className="block text-sm font-medium text-gray-800">Nom</label>
                        <input type="text" className="w-full border rounded px-2 py-1 text-gray-900" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} required />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-800">Note (sur 100)</label>
                        <input type="number" min="0" max="100" className="w-full border rounded px-2 py-1 text-gray-900" value={form.rating} onChange={e => setForm(f => ({ ...f, rating: e.target.value }))} required />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-800">Potentiel (sur 100)</label>
                        <input type="number" min="0" max="100" className="w-full border rounded px-2 py-1 text-gray-900" value={form.potential} onChange={e => setForm(f => ({ ...f, potential: e.target.value }))} required />
                      </div>
                      <div>
                        ...
                        >
                          {form.photo ? (
                            <img src={form.photo} alt="aperçu" className="mx-auto h-20 w-20 object-cover rounded-full mb-2" />
                          ) : (
                            <span className="text-gray-400">Glissez une image ici ou cliquez pour choisir un fichier</span>
                          )}
                          <input
                      ...
                      <button type="submit" className="w-full bg-blue-600 text-white rounded py-2 font-semibold hover:bg-blue-700 transition">Valider</button>
                    </form>
                  </div>
                </div>
              )}
